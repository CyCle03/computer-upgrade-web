...
| 생성 | 2026-06-09 · `scripts/build_map_extract_doc.py` |
...
> **해석 (맵 문자열 기준):** `1원 단위`·`1만원 단위`·`1코인 단위` = UI 표시 단위.  
> **`SP : N`** = SCA 코인 결제가. **`N코인`** (SCA 접두 없음, 파티 UI 등) = Normal 코인(가스, 천만 미네랄=1).  
> 상세: [`docs/original-map-v1.2.9.md`](original-map-v1.2.9.md) §2.1.1

## 3. SCA 상점 `SP :` 표기 (맵에 박힌 가격 문구)

`SP : M` = **SCA 코인 M개** 결제. 왼쪽 `N코인`은 상품 티어 라벨(Normal 코인 단위 표기).
...
> **`N코인`** (예: `2코인`, `20코인` …)은 §3 `SP :` 줄·파티 블록 **근접 문자열**에서도 확인됩니다. SCA 접두 없는 **`N코인` = Normal 코인(가스)** — [`original-map-v1.2.9.md`](original-map-v1.2.9.md) §2.1.1
