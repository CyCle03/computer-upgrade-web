...
> **스타 가스 UI** = 조립 PC **스펙** (재화 아님). **`N코인`** = **코인 재화** (천만 미네랄=1). **`SP : N`** = SCA 코인 결제가.  
> 상세: [`docs/original-map-v1.2.9.md`](original-map-v1.2.9.md) §2.1.1 · §2.2 · §2.3

## 3. SCA 상점 `SP :` 표기 (맵에 박힌 가격 문구)

`SP : M` = **SCA 코인 M개** 결제. 왼쪽 `N코인`은 상품 **티어 라벨**(코인 단위).
...
> **`N코인`** (예: `2코인`, `20코인` …) = **코인 재화** (가스=스펙과 별개) — [`original-map-v1.2.9.md`](original-map-v1.2.9.md) §2.3
