...
- **한국어/영어 전환 — `frontend/src/i18n.js`**
  ...
  - **서버가 내려준 오류 문구는 `translateServerError()` 를 거쳐 화면에 낸다.** 백엔드(`src/*.ts`)는 한국어 문장을 내려주는데, 예전에는 `err.message` 를 그대로 뿌려서 **영어 화면에 로그인 실패 문구만 한국어로** 나갔다. 지금은 여섯 자리(레이드 연결·SCA 구매·환생·계정 초기화·진행도 불러오기·로그인)가 이 함수를 거친다. 표에 없는 문장은 원문 그대로 나간다.
