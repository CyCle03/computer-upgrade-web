...
- **`main`에 push → self-hosted 러너(Oracle Cloud ARM, `web-game-server`)가 자동 배포** (`.github/workflows/deploy.yml`): `git reset --hard origin/main` → `npm run build` → `systemctl restart`. 서비스: <https://pc.elcherlab.com/> (구 <http://computer-upgrade-web.duckdns.org/>)
