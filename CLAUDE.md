...
- JSX 전부가 `<script type="text/babel">` 한 블록 안에 있음(3,700줄+). **구문 오류가 나면 블록 전체가 컴파일 실패**해 앱이 아예 안 뜬다(부분 실패 없음) — 그래서 로드만 돼도 "구문 OK" 신호가 된다.
...
- 컴포넌트 예: `HuntScene`(Canvas 2D 사냥 씬). 시각 레이어는 상태값을 읽어 그리기만 하고 게임 로직은 건드리지 않는 게 원칙.
...
```
npm test                    # hardware + upgradeProb + balance 스냅샷 (+ DB 설정 시 reward)
...
- **프론트는 타입체크가 없다**(Babel 인-브라우저) — index.html 편집 후엔 실제 로드로 확인.
