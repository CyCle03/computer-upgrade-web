...
| **한국어 · English** | 헤더 🌐 버튼으로 전환. 원문은 한국어이고 사전(`frontend/src/i18n.js`)이 영어를 덧입힌다. 설정은 localStorage 에만 저장하며, 서브도메인 사이에서는 `?lang=en` 링크로 넘긴다. 서버가 내려준 오류 문구도 화면 언어를 따른다 — 백엔드 문구는 `translateServerError()` 가 옮기고, 통합 인증에는 `?lang=` 을 넘겨 auth 가 직접 그 언어로 답한다 |
...
- 10층 단위 마일스톤마다 **누적 보상표 차분** 지급 (10층=1,000 · 20층=3,000 · … · 100층=80,000)
- 환생 수치 1,000만당 보상 +100% (`1 + rebirthStat / 10,000,000`)
- 이미 수령한 층 이하 재요청 시 거부
- 날짜가 바뀌면 `highestClaimedFloor`가 0으로 리셋
- 예: 오늘 80층까지 수령 후 100층 클리어 → 80,000 − 40,000 = **40,000코인** 차분 지급

> 프로덕션 DB에 옛 보상 RPC(10층당 10코인)가 남아 있으면 `scripts/migrate_raid_rewards.sql`을 적용한 뒤 Render를 재배포하세요.

### 열람권 문서 (`/internal/export-user`)

통합 인증의 "내 데이터 내려받기"가 부르는 내부 경로입니다. 문서는 **한국어·영어 두 벌**이고,
통합 인증이 본문에 실어 보낸 `lang` 이 `en` 이면 영어 키로 답합니다(모르는 값이면 한국어).
키를 그때그때 번역하지 않는 건 이게 받아서 보관하는 파일이라, 같은 키가 언어에 따라 달라지면
이미 받아 둔 파일과 비교가 안 되기 때문입니다. 게임 진행 상태·재화 같은 **행 자체는 DB 컬럼
이름 그대로**라 손대지 않습니다.
