...
async function main() {
  ...
  console.log('[TestRunner] balance snapshot regression...');
  if (!runScript('src/testBalanceSnapshot.ts')) {
    process.exit(1);
  }

  console.log('[TestRunner] raid reward table drift guard...');
  if (!runScript('src/testRewardTable.ts')) {
    process.exit(1);
  }
  ...
  console.log('[TestRunner] sca upgrade pure-logic unit tests...');
  if (!runScript('src/testScaUpgrades.ts')) {
    process.exit(1);
  }

  console.log('[TestRunner] raid reward payout formula unit tests...');
  if (!runScript('src/testRewardMath.ts')) {
    process.exit(1);
  }
