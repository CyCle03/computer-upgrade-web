...
function runHardwareTests() {
  ...
  const perfectParts: ComputerParts = {
    cpu: { manufacturer: 'Intel', level: 5, ddrGeneration: 'DDR3' }, // 코어 2, 요구 쿨링 450
    gpu: { level: 5 }, // 데미지: 100(GPU 5강 기본) * 4.8(CPU 5강 배율) = 480
    ram: { level: 4, clockMhz: 1600, capacityGb: 4, ddrGeneration: 'DDR3' }, // 공속: 44프레임/24 = 1.83초, 사냥배치 = 2기
    cooler: { level: 2, coolingCapacity: 650 }, // 쿨링 650 (발열 450 커버), 방어력 6
    motherboard: { socketManufacturer: 'Intel', supportedDdrGeneration: 'DDR3', shieldIncrease: 30 },
    storage: { type: 'SSD', capacityGb: 250 }, // 속도: 25 * 4 = 100MB/s
  };
  ...
  const cond1 = 
    specs1.unitLimit === 2 &&
    specs1.maxHuntingUnits === 2 &&
    specs1.unitDamage === 480 &&
    specs1.attackSpeedSec === 1.83 &&
    specs1.unitDefense === 6 &&
    specs1.downloadSpeedMb === 100 &&
  ...
  const overheatedParts: ComputerParts = {
    cpu: { manufacturer: 'AMD', level: 6, ddrGeneration: 'DDR5' }, // 요구 발열: 1100
    gpu: { level: 2 },
    ram: { level: 10, clockMhz: 4800, capacityGb: 8, ddrGeneration: 'DDR5' },
    cooler: { level: 2, coolingCapacity: 500 }, // 쿨링성능 500 (발열 1100 해소 불가)
    motherboard: { socketManufacturer: 'AMD', supportedDdrGeneration: 'DDR5', shieldIncrease: 8000 },
    storage: { type: 'HDD', capacityGb: 500 }, // 속도: 25 * 1 = 25MB/s
  };
  ...
  // ------------------------------------------------------------------------
  console.log('\n[TestCase 4] CPU/RAM/보드 간 DDR 혼용 (DDR 혼용 및 초당 50% HP Decay 검증)');
  const ddrMismatchedParts: ComputerParts = {
    cpu: { manufacturer: 'Intel', level: 6, ddrGeneration: 'DDR4' }, // CPU DDR4
    gpu: { level: 1 },
    ram: { level: 1, clockMhz: 1333, capacityGb: 1, ddrGeneration: 'DDR3' }, // DDR3 램 혼용 (CPU/보드와 미스매치)
    cooler: { level: 4, coolingCapacity: 1000 },
    motherboard: { socketManufacturer: 'Intel', supportedDdrGeneration: 'DDR4', shieldIncrease: 200 }, // 보드 DDR4
    storage: { type: 'SSD', capacityGb: 250 },
  };
  ...
  // DDR 2종류 혼용 시 초당 HP Decay 비율은 0.50 (50%)여야 함
  const cond4 = 
    specs4.penalties.isDdrMismatched && 
    specs4.penalties.hpDecayRate === 0.50;

  if (cond4) {
    console.log('=> [PASSED] DDR 혼용 오류 감지 및 초당 50% HP 감쇠 페널티 검증 성공');
  } else {
  ...
  // ------------------------------------------------------------------------
  console.log('\n[TestCase 5] CPU/RAM/보드 3개 세대 전원 불일치 (극단 혼용 및 초당 100% HP Decay 검증)');
  const extremeDdrParts: ComputerParts = {
    cpu: { manufacturer: 'Intel', level: 12, ddrGeneration: 'DDR5' }, // CPU DDR5
    gpu: { level: 1 },
    ram: { level: 5, clockMhz: 2400, capacityGb: 4, ddrGeneration: 'DDR4' }, // RAM DDR4
    cooler: { level: 4, coolingCapacity: 2000 },
    motherboard: { socketManufacturer: 'Intel', supportedDdrGeneration: 'DDR3', shieldIncrease: 200 }, // Board DDR3
    storage: { type: 'SSD', capacityGb: 250 },
  };
  ...
  // 3가지 세대가 전부 혼용 시 초당 HP Decay 비율은 1.00 (100%)이어야 함
  const cond5 = 
    specs5.penalties.isDdrMismatched && 
    specs5.penalties.hpDecayRate === 1.00;

  if (cond5) {
    console.log('=> [PASSED] 3세대 DDR 극단 혼용 감지 및 초당 100% HP 감쇠 페널티 검증 성공');
  } else {
